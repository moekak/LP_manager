...
class SystemFeedback{
   public $logFile = LOG_ERROR_FILE_PATH;
   public $user_id;
   ...
   public static function writeErrorLog($error_msg){
      ...
      $date = date("Y-m-d");
      // ファイル名を日付で設定
      $filename = LOG_ERROR_FILE_PATH . "log_" . $date . ".log";
   ...
   // エラーログ書き込み処理
   public static function catchError($error_msg){
      ...
      self::writeErrorLog($logMessage);
   }
  

   public static function handleError($errorCode, $errorMsg,  $redirectPath) {
    $_SESSION["error"] = ["errorCode" => $errorCode, "errorMsg" => $errorMsg];
    header("Location:" . ROUTE_PATH . $redirectPath);
    exit;
}
